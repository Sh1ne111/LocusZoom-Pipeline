## LocusZoom Pipeline visualization of GWAS (eQTL, EWAS TWAS and MWAS)  regional results for research and publication <a href="Fig/locuszoom.jpg"><img src="Fig/locuszoom.jpg" align="right" alt="logo" height="220" width="450" /></a>
...
> I have changed the output Figure for both PDF and PNG , and also the set color which will generate a random color (from the [RColorBrewer](https://cran.r-project.org/web/packages/RColorBrewer/index.html)  and add the [ggsci](https://nanx.me/ggsci/articles/ggsci.html)) for snpset in locuszoom.R.
> 
```linux
...
> Here, you need install the [BEDOPS](https://bedops.readthedocs.io/en/latest/index.html) to convert any type your file format to the [bed format](http://genome.ucsc.edu/FAQ/FAQformat.html#format1.7). Or you only need to prepare your data frame with: chr, start, end, name (type: exon, CDS and gene ETC), score, strand, thickStart, thickEnd and itemRgb (YOUR name color. One hand, I made a change that according to your name which will generates a corespondeing random color (from the [RColorBrewer](https://cran.r-project.org/web/packages/RColorBrewer/index.html) (and added the [ggsci](https://nanx.me/ggsci/articles/ggsci.html)) theme in locuszoom.R ) SO, you only need to prepare the above first four data frame is enough. One the other hand, I also CHANGED the gene text color (LOL). SO If you want to keep the same as the orignal locuszoom color, you just only to change the default.args for locuszoom [options](https://genome.sph.umich.edu/wiki/LocusZoom_Standalone).
...
$ ./bin/locuszoom --metal AGP50763.metal --refsnp chr4:578203670 --flank 15MB  --build bl38 --pop BARLEY --source 1000G_August2019 --no-cleanup --snpset SNP_density --bed-tracks ../examples/ref.flab.bed legend='right'
$ ./bin/locuszoom --metal HORVU0Hr1G000410.metal --refsnp chr3:641835852 --flank 15MB  --build bl38 --pop BARLEY --source 1000G_August2019 --no-cleanup --snpset SNP_density --bed-tracks ../examples/ref.flab.bed legend='right'
```

![HORVU7Hr1G119370](Fig/chr2_6981819-22981819_bed.png "HORVU7Hr1G119370")

![AGP50763](Fig/1.png "HORVU7Hr1G119370")

![HORVU0Hr1G000410](Fig/3.png "HORVU7Hr1G119370")
